...
class MReview
{
    ...
    public function feedBack(){
        $sql = 'SELECT users.username, reviews.review_id , reviews.user_id , reviews.product_id, reviews.comment, reviews.status, reviews.created_at  FROM reviews JOIN users ON reviews.user_id = users.user_id WHERE reviews.status = 1;`';
        $this->connect->setQuery($sql);
        return $this->connect->loadData();
    }
}
